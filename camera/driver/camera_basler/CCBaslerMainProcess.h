 ...
 *
 */
#ifndef CCBASLERMAINPROCESS_H
#define CCBASLERMAINPROCESS_H
...
class CCBaslerMainProcess
{
public:
    CCBaslerMainProcess(const std::string &, const std::string &, const std::string &, int, const std::string &, int transmitType, const nlohmann::json &json);
    ~CCBaslerMainProcess();
    bool openCamera(int triggerType);
    void run();
    std::shared_ptr<utility::CCMessageNode> rosNode();
    std::shared_ptr<CCBaslerCameraImp> cameraIO();
...
private:
    ...
    std::shared_ptr<CCBaslerCameraRosNode> 	m_node;
    std::shared_ptr<CCBaslerCameraImp>      m_camera;
