 ...
 *
 */
#ifndef CC_BASLER_CAMERA_ROSNODE_H
#define CC_BASLER_CAMERA_ROSNODE_H
...
class CCBaslerCameraRosNode : public utility::CCMessageNode
{
public:
    explicit CCBaslerCameraRosNode(const std::string &, int transmitType);
    ...
    template<typename CallbackT>
    void     createSubscribeSetParam( const std::string &topic
                                      , CallbackT && callback
                                      , uint32_t queueSize = 1);
...
template<typename CallbackT>
void CCBaslerCameraRosNode::createSubscribeGetImage(  const std::string& topic
                                                     , CallbackT && callback
                                                     , uint32_t queueSize)
{
...
template<typename CallbackT>
void CCBaslerCameraRosNode::createSubscribeSetParam( const std::string &topic
                                                     , CallbackT && callback
                                                     , uint32_t queueSize)
{
...
#endif // CC_BASLER_CAMERA_ROSNODE_H
