...
std::shared_ptr<utility::CCDeviceImpl> startDevice(const std::list<std::string> &listString, int transmitType, bool &ret)
{
    ...
    if (listString.size() < 4) {
        CCError(u8"Basler相机startDevice失败", "驱动参数有错误，请检查device.json中相机相关的参数");
        return nullptr;
    }
    ...
    auto camera = std::make_shared<CCBaslerCamera>( mainType
                                                    , subType
                                                    , deviceId
                                                    , cameraId
                                                    , info
                                                    , transmitType
                                                    , jsonObj);
    ret = camera->openCamera(triggerType);
...
{
    CCInfo("basler camera %s enter", __FUNCTION__ );
...
#ifdef BASLER_USB_AREA
    type.subType = "camera_basler_usb_area";
    type.subTypeName = "Basler Usb Camera";
#else
    type.subType = "camera_basler_gige_area";
    type.subTypeName = "Basler GigE Camera";
#endif
    type.deviceExeName = "basler";
    type.startDevice = startDevice;

    CCInfo("basler camera %s exit", __FUNCTION__ );
    return 0;
}

CCBaslerCamera::CCBaslerCamera(const std::string &mainType,
...
{
    CCInfo("basler camera %s enter", __FUNCTION__ );
    m_process = std::make_shared<CCBaslerMainProcess>(mainType, subType, objId, index, info, transmitType, json);
    CCInfo("basler camera %s exit", __FUNCTION__ );
}

std::shared_ptr<utility::CCMessageNode> CCBaslerCamera::rosNode()
{
    CCInfo("basler camera %s enter", __FUNCTION__ );
    return m_process->rosNode();
}

bool CCBaslerCamera::openCamera(int triggerType)
{
    CCInfo(" basler camera %s enter", __FUNCTION__ );
    return m_process->openCamera(triggerType);
}

std::shared_ptr<CCBaslerCameraImp> CCBaslerCamera::getCameraHandle()
{
    CCInfo("basler camera %s enter", __FUNCTION__ );
    return m_process->cameraIO();
}

void CCBaslerCamera::run()
{
    m_process->run();
}

void CCBaslerCamera::registerDataIdValidator(void *dataIdMeta)
{
