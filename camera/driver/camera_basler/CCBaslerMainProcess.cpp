...
CCBaslerMainProcess::CCBaslerMainProcess(const std::string &mainType
    ...
    , m_camera(std::make_shared<CCBaslerCameraImp>(index, info))
    , m_lens(nullptr)
    , m_lensEnabled(false)
{
    ...
    m_camera->setImageCapturedCallback(std::bind(&CCBaslerMainProcess::onImageCaptured, this, _1, _2, _3, _4));

    initializeLens(json);
    std::string nodeName = utility::ccRosNodeName(type, objId);
    // nodeName = "driver_" + nodeName;
    m_node = std::make_shared<CCBaslerCameraRosNode>(nodeName, transmitType);

    std::string topicName = driver::ccRosTopicNameDriverImageData(type, objId);
    CCInfo("basler camera created param topic: %s", topicName.c_str());
    m_node->createPublishImage(topicName, 1024);

    topicName = driver::ccRosTopicNameGetImage(type, objId);
    CCInfo("basler camera created image subscribe: %s", topicName.c_str());
    m_node->createSubscribeGetImage(topicName,
                                    std::bind(&CCBaslerMainProcess::onSubscribeGetImage, this, _1), 1024);

    topicName = driver::ccRosTopicNameSetParamBack(type, objId);
    CCInfo("basler camera created param subscribe: %s", topicName.c_str());
    ...
    m_node->createSubscribeSetParam(topicName,
                                    std::bind(&CCBaslerMainProcess::onSubscribeSetParam, this, _1), 1024);
...
CCBaslerMainProcess::~CCBaslerMainProcess()
{
    // 关闭镜头设备
...
bool CCBaslerMainProcess::openCamera(int triggerType)
{
    CCInfo("%s enter.", __FUNCTION__);
...
std::shared_ptr<utility::CCMessageNode> CCBaslerMainProcess::rosNode()
{
    return m_node;
}

std::shared_ptr<CCBaslerCameraImp>   CCBaslerMainProcess::cameraIO()
{
    return m_camera;
}


void CCBaslerMainProcess::onSubscribeGetImage(const CCGetImage::SharedPtr getImage)
{
...
void CCBaslerMainProcess::onSubscribeSetParam(const CCCameraParam::SharedPtr param)
{
    setCameraParam(*param);
}

void CCBaslerMainProcess::setCameraParam(const CCCameraParam &param)
{
    CCInfo("%s enter.", __FUNCTION__);

    CCDebug("basler set back param");
    ...
    *paramBack = param;
    if (m_camera == nullptr || !m_camera->isDeviceAccessible()) {
       CCError(u8"basler相机[%d]设置参数失败", u8"请检查相机网线、USB线、电源线两头连接是否正常", m_cameraId);
       m_node->publishParam(paramBack);
       return;
    }

    CCDebug("basler set param begin");
    ...
    for (const auto &paramItem : param.item) {
        ...
        case CCCameraParamItem::TYPE_FLOAT:
            if (paramItem.value_float > -ZERO &&
                ...
                m_camera->setDoubleValue(featureName.c_str(), paramItem.value_float);
        ...
        case CCCameraParamItem::TYPE_ENUM:
            if (!paramItem.value_str.empty() && paramItem.value_str != it->value_str) {
                m_camera->setEnumValue(featureName.c_str(), paramItem.value_str.c_str());
                it->value_str = paramItem.value_str;
            }
            else if (paramItem.value_str.empty() && paramItem.value_int != it->value_int) {
                uint32_t value = static_cast<uint32_t>(paramItem.value_int);
                m_camera->setEnumValue(featureName.c_str(), value);
                it->value_int = paramItem.value_int;
            }
            break;
    ...
    CCDebug("%s set param back", m_objId.c_str());
    ...
    // 返回参数
    CCDebug("%s publish param back", m_objId.c_str());
    m_node->publishParam(paramBack);

    // 结束设置参数时间
    CCDebug("%s set param time", m_objId.c_str());
...
void CCBaslerMainProcess::onImageCaptured(void *buffer, int width, int height, int channel)
{
    CCInfo("%s enter.", __FUNCTION__);
    CC_ASSERT(buffer);
    ...
    if (m_imageIndex) {
        ...
        // 开始收图时间
        ccImage->header.times.emplace_back(4);
        ccImage->header.times.emplace_back(CCDateTime::now().toMSecsSinceEpoch());
    }
    CCInfo("%s exit %s.", __FUNCTION__, ccImage->header.node_id.c_str());
}

bool CCBaslerMainProcess::openLens()
{
    CCInfo("%s enter.", __FUNCTION__);
...
std::shared_ptr<driver::CCAbstractLens> CCBaslerMainProcess::loadLens(const std::string &fileName)
{
...
bool CCBaslerMainProcess::initializeLens(const nlohmann::json &json)
{
    CCInfo("%s enter.", __FUNCTION__);
...
bool CCBaslerMainProcess::closeLens()
{
    CCInfo("%s enter.", __FUNCTION__);
