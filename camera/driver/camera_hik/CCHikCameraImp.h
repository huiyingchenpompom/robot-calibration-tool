#ifndef CCHIKCAMERAIMP_H
#define CCHIKCAMERAIMP_H
...
{
public:
    explicit CCHikCameraImp(int cameraId, const std::string &cameraInfo);
    virtual ~CCHikCameraImp();
    ...
    bool enumDevices(std::map<std::string, MV_CC_DEVICE_INFO> &cameraInfoList);
    bool getCameraDeviceNum(unsigned int &deviceNum);
    bool initDeviceByAdress(const std::string &ipAddress);
    ...
    /* 以下是设备触发控制接口 */
    bool setHardwareTrigger(HaikangTriggerSource triggerSource
                            = TriggerSourceLine0);      /* 设置触发模式为硬件(外部)触发 */
    ...
    bool setPixelFormat(PixelFormat pixelFormat);       /* 设置相机像素格式 */
    ...
    bool isValidPixelFormat(MvGvspPixelType type);
    bool isColor(MvGvspPixelType type);
...
public:
    ...
    bool setEnumValue(const std::string &featureName, unsigned int value);
    bool setBoolVaule(const std::string &featureName, bool value);
    bool setStringValue(const std::string &featureName, const std::string &value);
...
private:
    ...
    bool getEnumValue(const std::string &featureName, unsigned int &value);
    bool getBoolVaule(const std::string &featureName, bool &value);
    bool getStringValue(const std::string &featureName, std::string &value);
...
protected:
    ...
    /* 打印相机参数配置信息 */
    template<typename T>
    bool printCameraSetParamInfo(const std::string &featureName, T value, const std::string &action, int status);
