...
CCHikMainProcess::CCHikMainProcess(  const std::string &mainType
    ...
    , m_camera(std::make_shared<CCHikCameraImp>(index, info))
    , m_lens(nullptr)
    , m_lensEnabled(false)
{
    ...
    m_camera->setImageCapturedCallback(std::bind(&CCHikMainProcess::onImageCaptured, this, _1, _2, _3, _4));

    initializeLens(json);
    std::string nodeName = utility::ccRosNodeName(type, objId);
    // nodeName = "driver_" + nodeName;
    m_node = std::make_shared<CCHikCameraRosNode>(nodeName, transmitType);

    std::string topicName = driver::ccRosTopicNameDriverImageData(type, objId);
    CCInfo("hik camera created param topic: %s", topicName.c_str());
    m_node->createPublishImage(topicName, 1024);

    topicName = driver::ccRosTopicNameGetImage(type, objId);
    CCInfo("hik camera created image subscribe: %s", topicName.c_str());
    m_node->createSubscribeGetImage(topicName,
                                    std::bind(&CCHikMainProcess::onSubscribeGetImage, this, _1), 1024);

    topicName = driver::ccRosTopicNameSetParamBack(type, objId);
    CCInfo("hik camera created param subscribe: %s", topicName.c_str());
    ...
    m_node->createSubscribeSetParam(topicName,
                                    std::bind(&CCHikMainProcess::onSubscribeSetParam, this, _1), 1024);
...
CCHikMainProcess::~CCHikMainProcess()
{
    // 关闭镜头设备
...
bool CCHikMainProcess::openCamera(int triggerType)
{
    CCInfo("%s enter.", __FUNCTION__);
    m_getImage->trigger_type = triggerType;
    bool result = true;
#ifdef HIK_USB_AREA
    result &= m_camera->createHandle();
    result &= m_camera->registerImageCallBack();
    ...
    result &= m_camera->startGrabbing();     /* 新增，设置延迟发包间隔为10，可以降低相机采集速度*/
#else
    result &= m_camera->createHandle();
    result &= m_camera->registerImageCallBack();
    result &= m_camera->openDevice();
...
std::shared_ptr<utility::CCMessageNode> CCHikMainProcess::rosNode()
{
    return m_node;
}

std::shared_ptr<CCHikCameraImp>   CCHikMainProcess::cameraIO()
{
    return m_camera;
}

void CCHikMainProcess::onSubscribeGetImage(const CCGetImage::SharedPtr getImage)
{
...
void CCHikMainProcess::onSubscribeSetParam(const CCCameraParam::SharedPtr param)
{
    setCameraParam(*param);
}

void CCHikMainProcess::setCameraParam(const CCCameraParam &param)
{
    CCInfo("%s enter.", __FUNCTION__);
    ...
    if (param.params.size() == param.values.size()) {
        for (uint8_t i = 0; i < param.params.size(); i++) {
...
#ifdef HIK_GIGE_LINE
                // 线扫行频
                if (cameraParam[CCCameraParam::PARAM_LINERATE] > -ZERO
                    && std::fabs(cameraParam[CCCameraParam::PARAM_LINERATE] - m_cameraParam[CCCameraParam::PARAM_LINERATE]) > ZERO) {
                    m_camera->setLineRate(cameraParam[CCCameraParam::PARAM_LINERATE]);
                    m_cameraParam[CCCameraParam::PARAM_LINERATE] = cameraParam[CCCameraParam::PARAM_LINERATE];
                }
#endif
            }
            // 焦距
                ...
                CCInfo(u8"驱动获取到焦距参数：%d", static_cast<int>(cameraParam[CCCameraParam::PARAM_FOCUS]));
                if (cameraParam[CCCameraParam::PARAM_FOCUS] > -ZERO
                    ...
                    if(m_lensEnabled) {
                        CCInfo(u8"驱动设置焦距参数：%d", static_cast<int>(cameraParam[CCCameraParam::PARAM_FOCUS]));
            ...
            // 光圈
                ...
                CCInfo(u8"驱动获取到光圈参数：%d", static_cast<int>(cameraParam[CCCameraParam::PARAM_IRIS]));
                if (cameraParam[CCCameraParam::PARAM_IRIS] > -ZERO
                    ...
                    if(m_lensEnabled){
                        CCInfo(u8"驱动设置光圈参数：%d", static_cast<int>(cameraParam[CCCameraParam::PARAM_IRIS]));
...
void CCHikMainProcess::onImageCaptured(void *buffer, int width, int height, int channel)
{
    CCInfo("%s enter.", __FUNCTION__);
    CC_ASSERT(buffer);
...
bool CCHikMainProcess::openLens()
{
    CCInfo("%s enter.", __FUNCTION__);
...
std::shared_ptr<driver::CCAbstractLens> CCHikMainProcess::loadLens(const std::string &fileName)
{
...
bool CCHikMainProcess::initializeLens(const nlohmann::json &json)
{
    CCInfo("%s enter.", __FUNCTION__);
...
bool CCHikMainProcess::closeLens()
{
    CCInfo("%s enter.", __FUNCTION__);
