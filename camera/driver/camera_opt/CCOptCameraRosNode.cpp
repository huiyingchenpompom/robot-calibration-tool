...
CCOptCameraRosNode::CCOptCameraRosNode(const std::string &nodeName, int transmitType)
...
void CCOptCameraRosNode::createPublishImage(const std::string &topic, uint32_t queueSize)
{
    m_publisherImage = this->create_publisher<CCImage>(topic, queueSize);
}

void CCOptCameraRosNode::publishImage(const CCImage::SharedPtr image)
{
...
void CCOptCameraRosNode::createPublishParam(const std::string &topic, uint32_t queueSize)
{
    CCInfo("Create camera param topic: %s", topic.c_str());
    m_publisherParam = this->create_publisher<CCCameraParam>(topic, queueSize);
}

void CCOptCameraRosNode::publishParam(const ros_msg::msg::CCCameraParam::SharedPtr param)
{
