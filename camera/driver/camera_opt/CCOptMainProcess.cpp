...
CCOptMainProcess::CCOptMainProcess(  const std::string &mainType
    ...
    , m_camera(std::make_shared<CCOptCameraImp>(index, info))
{
    ...
    m_camera->setImageCapturedCallback(std::bind(&CCOptMainProcess::onImageCaptured, this, _1, _2, _3, _4));

    std::string nodeName = utility::ccRosNodeName(type, objId);
    nodeName = "driver_" + nodeName;
    m_node = std::make_shared<CCOptCameraRosNode>(nodeName, transmitType);

    std::string topicName = driver::ccRosTopicNameDriverImageData(type, objId);
    CCInfo("opt camera created param topic: %s", topicName.c_str());
    m_node->createPublishImage(topicName, 1024);

    topicName = driver::ccRosTopicNameGetImage(type, objId);
    CCInfo("opt camera created image subscribe: %s", topicName.c_str());
    m_node->createSubscribeGetImage(topicName,
                                    std::bind(&CCOptMainProcess::onSubscribeGetImage, this, _1), 1024);

    topicName = driver::ccRosTopicNameSetParamBack(type, objId);
    CCInfo("opt camera created param subscribe: %s", topicName.c_str());
    ...
    m_node->createSubscribeSetParam(topicName,
                                    std::bind(&CCOptMainProcess::onSubscribeSetParam, this, _1), 1024);
...
bool CCOptMainProcess::openCamera(int triggerType)
{
    CCInfo("%s enter.", __FUNCTION__);
...
std::shared_ptr<utility::CCMessageNode> CCOptMainProcess::rosNode()
{
    return m_node;
}

std::shared_ptr<CCOptCameraImp>   CCOptMainProcess::cameraIO()
{
    return m_camera;
}

void CCOptMainProcess::onSubscribeGetImage(const CCGetImage::SharedPtr getImage)
{
...
void CCOptMainProcess::onSubscribeSetParam(const CCCameraParam::SharedPtr param)
{
    setCameraParam(*param);
}

void CCOptMainProcess::setCameraParam(const CCCameraParam &param)
{
    CCInfo("%s enter.", __FUNCTION__);
...
void CCOptMainProcess::onImageCaptured(void *buffer, int width, int height, int channel)
{
    CCInfo("%s enter.", __FUNCTION__);
    CC_ASSERT(buffer);
